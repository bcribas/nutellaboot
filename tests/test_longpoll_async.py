...
async def test_lock_reaches_machine_in_under_a_second(data_root, image_testes3):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://t") as c:
        await c.post(f"/api/v1/images/testes3/machines/{MAC}/status", json={}, headers=hm)

        t0 = time.monotonic()
        poll = asyncio.create_task(
            c.get(f"/api/v1/images/testes3/machines/{MAC}/commands?wait=25", headers=hm)
        )
        await asyncio.sleep(0.2)  # o agente já está pendurado esperando
        await c.post("/api/v1/images/testes3/lock", headers=hi)
...
async def test_many_machines_wait_without_burning_cpu(data_root, image_testes3):
    ...
    async with httpx.AsyncClient(
        ...
        r = await c.post("/api/v1/images/testes3/lock", headers=hi)
