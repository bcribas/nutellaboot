...
def img(data_root, admin_key):
    ...
    return store.create_site_image("25brbr", "Sede de teste", "t")
...
def test_credentials_devolve_tudo(client, img, ha):
    r = client.get("/api/v1/site-images/25brbr/credentials", headers=ha)
...
def test_credentials_nao_rotaciona(client, img, ha):
    ...
    antes = client.get("/api/v1/site-images/25brbr/credentials", headers=ha).json()["token"]
    depois = client.get("/api/v1/site-images/25brbr/credentials", headers=ha).json()["token"]
    assert antes == depois == img["token"]


def test_credentials_exige_admin(client, img):
    # sem chave nenhuma
    assert client.get("/api/v1/site-images/25brbr/credentials").status_code == 401
    # com o token da própria imagem NÃO basta: credenciais é coisa de admin
    r = client.get(
        "/api/v1/site-images/25brbr/credentials",
...
def test_credentials_404_imagem_inexistente(client, ha):
    assert client.get("/api/v1/site-images/naoexiste/credentials", headers=ha).status_code == 404
...
def test_link_de_criacao_ja_inclui_token(client, admin_key, data_root):
    ...
    r = client.post(
        "/api/v1/site-images",
        json={"id": "univap", "fullname": "Univap", "model": "t"},
